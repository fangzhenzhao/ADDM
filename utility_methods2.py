...
#------------------------------------------------------------------------------------------------------------------------
def detect_ood_svm_2(in_org_model, in_model, in_mix_id_odd, in_extra_data, in_del_index, delete_index=False, input_preprocessing=False):
    if input_preprocessing==False:
        per_in_mix_id_odd = in_mix_id_odd
    else:
        pred_labels= np.argmax( in_org_model.predict(in_mix_id_odd), axis=1 )
    ...
    if  delete_index == True:
        del_index = in_del_index
        features = in_model[0].predict(per_in_mix_id_odd)
        arr1 = postprocess_features(features)

        for i in range(len(del_index)):
            if i == 0:
                arr2 = np.delete(arr1,del_index[i],axis=1)
            else:
                arr2 = np.delete(arr2,del_index[i],axis=1)
            features_0 = arr2
            features_vector = features_0
